...
/**
 * Created by habi.ravi on 10/2/2014.
 */
public class ExchangeAppTestCase {

    @Test(dataProvider = "testExchangeAppDataProvider")
    public void testExchangeApp(Hashtable<String, String> data)
            throws InterruptedException, MalformedURLException {

        if (!TestUtil.isTestCaseExecutable("testExchangeApp",
                Excel.getInstance())) {
            DriverConfiguration.APPLICATION_LOGS
                    .debug("Skipping the test case :" + " testExchangeApp "
        ...
        try {
          ...
          Keywords.getKeywordInstance().switchToWindowWithTitle(DriverConfiguration.getDriverInstance().driver1,"Chat");
           ...
           //Click on app1
            Keywords.getKeywordInstance().clickOnLinkbyXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().sendApp1);

            Thread.sleep(5000);
            int agentAppCount = Keywords.getKeywordInstance().GetAgentAppElementCount(DriverConfiguration.getDriverInstance().driver1);

            System.out.println("Agent app count = " + agentAppCount);
            ...
            int agentAppCount1 = Keywords.getKeywordInstance().GetVisitorAppElementCount(DriverConfiguration.getDriverInstance().driver1);
            System.out.println("Visitor app count = " + agentAppCount1);
            Thread.sleep(5000);
            Assert.assertEquals(agentAppCount, agentAppCount1,
                    Messages.getMessage().stringAssertionFailureMessage);
            //Switch to Agent Chat window;
            DriverConfiguration.getDriverInstance().driver1.switchTo().window(MainWindow);
            Thread.sleep(2000);
            List<String> agentAppSource = Keywords.getKeywordInstance().GetAppTextSource(DriverConfiguration.getDriverInstance().driver1, "chat_win");
            System.out.println("Agent app text:\n"+agentAppSource);


            Keywords.getKeywordInstance().switchToWindowWithTitle(DriverConfiguration.getDriverInstance().driver1,"Chat");
            Thread.sleep(2000);
            List<String> visitorAppSource = Keywords.getKeywordInstance().GetAppTextSource(DriverConfiguration.getDriverInstance().driver1, "ClientView");
            System.out.println("Visitor app text:\n"+visitorAppSource);
            Thread.sleep(2000);

            Assert.assertEquals(agentAppSource, visitorAppSource, Messages.getMessage().stringAssertionFailureMessage);
            ...
            System.out.println("!!!!! TC Failed - testExchangeApp !!!!!");

        }
        System.out.println("Executed TC - testExchangeApp.");
        DriverConfiguration.APPLICATION_LOGS.debug("$$$$"
    ...
    @DataProvider
    public Object[][] testExchangeAppDataProvider() {
        return TestUtil.getdata("testExchangeApp",
                Excel.getInstance());
    }
}
