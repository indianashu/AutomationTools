 ...
 */
public class VisitorDispositionFormTestCase {
    // @Test(dataProvider = "testVisitorDispositionFormDataProvider")
    @Test(dataProvider = "testVisitorDispositionFormLongTextDataProvider")
    public void testVisitorDispositionForm(Hashtable<String, String> data)
            throws InterruptedException, MalformedURLException {

        if (!TestUtil.isTestCaseExecutable("testVisitorDispositionForm",
                Excel.getInstance())) {
            DriverConfiguration.APPLICATION_LOGS
                    .debug("Skipping the test case :" + " testVisitorDispositionForm "
        ...
        try {
            ...
            Keywords.getKeywordInstance().clickOnButton(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().closeButton);
            Thread.sleep(3000);
            //Click Quit Button
            Keywords.getKeywordInstance().clickOnButton(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().exitButton);
            // Thread.sleep(1000);
            Alert acceptExit = DriverConfiguration.getDriverInstance().driver1.switchTo().alert();
            acceptExit.accept();
            Thread.sleep(5000);
            Keywords.getKeywordInstance().selctItemfromDropDown(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().visitorSuggestionDropdown, data.get(Locator.getLocatorInstance().visitorSuggestion));
            Thread.sleep(3000);
            Keywords.getKeywordInstance().enterDataInTextFieldByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().feedbackArea, data.get(Locator.getLocatorInstance().feedback));
            Thread.sleep(3000);
            Keywords.getKeywordInstance().enterDataInTextFieldByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().visitorEmailId, data.get(Locator.getLocatorInstance().visitorEmail));
            Thread.sleep(3000);
            Keywords.getKeywordInstance().clickOnButtonByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().doneButton);
            Thread.sleep(10000);
            ...
            System.out.println("!!!!! TC Failed - testVisitorDispositionForm !!!!!");

        }
        System.out.println("Executed TC - testVisitorDispositionForm.");
        DriverConfiguration.APPLICATION_LOGS.debug("$$$$"
    ...
    @DataProvider
    public Object[][] testVisitorDispositionFormDataProvider() {
        return TestUtil.getdata("testVisitorDispositionForm",
                Excel.getInstance());
    }

    @DataProvider
    public Object[][] testVisitorDispositionFormLongTextDataProvider() {
        return TestUtil.getdata("testVisitorDispositionFormLongText",
                Excel.getInstance());
    }
