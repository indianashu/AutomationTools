 ...
 */
public class SearchForAppsTestCase {

    @Test(dataProvider = "testSearchForAppsDataProvider")
    public void testSearchForApps(Hashtable<String, String> data)
            throws InterruptedException, MalformedURLException {

        if (!TestUtil.isTestCaseExecutable("testSearchForApps",
                Excel.getInstance())) {
            DriverConfiguration.APPLICATION_LOGS
                    .debug("Skipping the test case :" + " testSearchForApps "
        ...
        try {
            ...
            Keywords.getKeywordInstance().clickOnDropdownByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().appButton);
            Thread.sleep(1000);
            Keywords.getKeywordInstance().enterDataInTextFieldByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().searchApps, "Mob");
            Thread.sleep(2000);


            int countOfApp = Keywords.getKeywordInstance().countOfElementPresentInPanelByXpath(DriverConfiguration.getDriverInstance().driver1, ".//*[@id='dashboard_id']/div[2]/div[5]/div[1]/ol");
            System.out.println("count = " + countOfApp);

            Keywords.getKeywordInstance().clearDataInTextFieldByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().searchApps);

            Keywords.getKeywordInstance().enterDataInTextFieldByXpath(DriverConfiguration.getDriverInstance().driver1, Locator.getLocatorInstance().searchApps, "XMob");
            Thread.sleep(2000);


            int countOfApp1 = Keywords.getKeywordInstance().countOfElementPresentInPanelByXpath(DriverConfiguration.getDriverInstance().driver1, ".//*[@id='dashboard_id']/div[2]/div[5]/div[1]/ol");
            System.out.println("count = " + countOfApp1);
            double actualCountOfApp = Double.parseDouble(data.get(Locator.getLocatorInstance().countOfApp));
            double actualCountOfApp1 = Double.parseDouble(data.get(Locator.getLocatorInstance().countOfApp1));

            Assert.assertEquals((int) actualCountOfApp, countOfApp, Messages.getMessage().stringAssertionFailureMessage);
            Assert.assertEquals((int) actualCountOfApp1, countOfApp1, Messages.getMessage().stringAssertionFailureMessage);
            ...
            System.out.println("!!!!! TC Failed - testSearchForApps !!!!!");

        }
        System.out.println("Executed TC - testSearchForApps.");
        DriverConfiguration.APPLICATION_LOGS.debug("$$$$"
    ...
    @DataProvider
    public Object[][] testSearchForAppsDataProvider() {
        return TestUtil.getdata("testSearchForApps",
                Excel.getInstance());
    }
}
